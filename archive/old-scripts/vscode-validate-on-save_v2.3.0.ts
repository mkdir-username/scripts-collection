 ...
 */
function extractErrorField(message: string): string | null {
...
function formatIssue(
  ...
  if (issue.sourceFile) {
    console.log(`      Module: ${basename(issue.sourceFile)}`);
  }
...
function formatOutput(
  ...
  console.log(
    `🔄 PROCESSING: ${fileName}${fileFormat === 'j2.java' ? ' [Jinja2/Java Template]' : ''}`
  );
  console.log('━'.repeat(80));
  console.log('');
  ...
  if (jinjaParseResult) {
    ...
    console.log(
      `   • Control structures: ${jinjaParseResult.stats.controlCount}`
    );
    console.log(
      `   • Total size: ${(jinjaParseResult.stats.totalSizeBytes / 1024).toFixed(2)} KB`
    );
    console.log(
      `   • Parse time: ${jinjaParseResult.stats.parseTimeMs.toFixed(2)}ms`
    );
    ...
    if (jinjaParseResult.imports.length > 0) {
      ...
      for (const imp of jinjaParseResult.imports) {
        console.log(`     - ${imp.description} (${basename(imp.resolvedPath)})`);
      }
    }
  ...
  if (stats) {
    ...
    console.log(
      '   ' + renderProgressBar(stats.totalComponents, stats.totalComponents)
    );
    console.log(`   ✅ Completed in ${stats.duration.toFixed(2)}s`);
    if (stats.positionMapBuildTime > 0) {
      console.log(
        `   📍 Position map built in ${stats.positionMapBuildTime.toFixed(2)}ms`
      );
    }
    console.log('');
  }
  ...
  if (report.versions) {
    const totalComps = `${report.versions.totalComponents} total (${Object.entries(
      report.versions.byVersion
    )
      .map(([v, c]) => `${v}: ${c}`)
      .join(', ')})`;
  ...
  if (report.errors && report.errors.length > 0) {
    console.log('━'.repeat(80));
    console.log(
      `❌ ERRORS: ${report.errors.length} critical issue${
  ...
  if (report.warnings && report.warnings.length > 0) {
    console.log('━'.repeat(80));
    console.log(
      `⚠️  WARNINGS: ${report.warnings.length} issue${
...
async function validateFile(filePath: string): Promise<void> {
  ...
  try {
    ...
    console.log(
      `   ✅ Mapped ${positionMap.byPointer.size} locations in ${posMapTime}ms`
    );
    console.log('');
