...
class SDUIWebValidator:
    ...
    def find_component_schema(self, component_type: str) -> Optional[Path]:
        ...
        # Специальная обработка для ConstraintWrapper
        ...
        # Ищем схемы и выбираем ту, которая поддерживает WEB
        all_matches = []
        ...
        # Выбираем схему с поддержкой WEB (предпочитаем v1 для ImageView)
        web_supported = []
        for schema_path in sorted(all_matches):
        ...
        # Возвращаем первую схему с поддержкой WEB
        if web_supported:
            return web_supported[0]

        # Если нет схемы с поддержкой WEB, возвращаем последнюю
    ...
    def validate_contract(self, contract_path: str) -> bool:
        ...
        # Большой разделитель для начала нового лога
        print("\n" * 3)
        print("╔" + "═" * 78 + "╗")
        print("║" + " " * 78 + "║")
        print(f"║  📋 ВАЛИДАЦИЯ СОВМЕСТИМОСТИ С WEB v3.0 | {timestamp}" + " " * 29 + "║")
        print("║" + " " * 78 + "║")
        print("╚" + "═" * 78 + "╝")
        ...
        # Визуальный разделитель для отделения выводов
        print("▓" * 80)
        print("█" * 80)
