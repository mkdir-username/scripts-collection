...
class AdvancedJSONLineMapper:
    ...
    def _find_key_exact(self, full_path: str, path_parts: list) -> Optional[int]:
        ...
        # Собираем родительские ключи для проверки контекста
        parent_keys = [p for p in path_parts[:-1] if isinstance(p, str)]
        ...
        for line_num, line in enumerate(self.content_lines, 1):
            if re.search(key_pattern, line):
                ...
                # Ищем родительские ключи в обратном порядке (ближайшие важнее)
                for i in range(line_num - 1, max(0, line_num - 50), -1):
                    check_line = self.content_lines[i]
                    for parent_key in reversed(parent_keys):
                        if f'"{parent_key}"' in check_line and parent_key not in found_parents:
                            # Проверяем, что это ключ, а не значение
                            if re.search(rf'"{parent_key}"\s*:', check_line):
                                found_parents.add(parent_key)
                                # Чем ближе родитель, тем больше вес
                                distance_weight = 50 - (line_num - i - 1)
                                context_score += distance_weight

                # Штраф если не все родители найдены
                missing_parents = len(parent_keys) - len(found_parents)
                context_score -= missing_parents * 100
...
def main():
    ...
    if '/SDUI/' in contract_path and not '/_JSON/' in contract_path:
