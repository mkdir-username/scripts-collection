...
const PROJECT_ROOT =
  process.env.PROJECT_ROOT || '/Users/username/Documents/front-middle-schema';
...
function buildPositionMap(
  ...
  try {
    ...
    function traverse(node: jsoncParser.Node, path: string = ''): void {
      if (!node || node.offset === undefined) return;
 ...
 */
function getPathHierarchy(path: string): string[] {
  if (!path) return [];
...
function formatIssue(
  ...
  icon: string,
  filePath: string,
  positionMap: Map<string, number> | null = null
): void {
  ...
  console.log(`      Link: ${filePath}#${lineNumber}`);
  console.log('');
}
...
function formatOutput(
  ...
  report: any | null,
  parseError?: string,
  stats?: { duration: number; totalComponents: number },
  positionMap: Map<string, number> | null = null
): void {
