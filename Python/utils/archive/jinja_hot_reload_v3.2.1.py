#!/usr/bin/env python3
"""
Jinja Hot Reload v3.2.1 - SMART режим с улучшенным форматированием логов

Новые возможности в v3.2.1:
1. 📊 Улучшенное форматирование вывода с визуальными разделителями
2. 🎨 Чёткое разделение между файлами и событиями
3. 📝 Структурированное отображение этапов обработки
4. 🔍 Лучшая читаемость при обработке множества файлов
...
3. 🎯 Умная обработка trailing/missing commas
4. 🛡️ Безопасный fallback на исходный JSON при критических ошибках

Запуск:
  python3 jinja_hot_reload_v3.2.1.py --smart          # Smart режим
  python3 jinja_hot_reload_v3.2.1.py --smart --debug  # Smart + Debug
"""
...
class EnhancedJinjaJsonPreprocessor:
    ...
    def clean_mixed_syntax(self, content: str) -> Tuple[str, Dict[str, str]]:
        """
...
class JinjaHotReloaderV32(FileSystemEventHandler):
    """Hot Reload v3.2.0 с интеллектуальным режимом"""
    ...
    def process_jj_file(self, file_path: Path):
        ...
        try:
            ...
            # 2. Очищаем смешанный синтаксис (с smart фиксами)
            cleaned_content, jinja_blocks = self.preprocessor.clean_mixed_syntax(content)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Jinja Hot Reload v3.2.1 - SMART режим с улучшенным форматированием'
    )
