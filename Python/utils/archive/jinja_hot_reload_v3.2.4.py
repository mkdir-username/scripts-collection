#!/usr/bin/env python3
"""
Jinja Hot Reload v3.2.4 - Отслеживание зависимостей импортов

Новые возможности в v3.2.4:
...
Запуск:
  python3 jinja_hot_reload_v3.2.4.py --smart          # Smart режим
  python3 jinja_hot_reload_v3.2.4.py --smart --debug  # Smart + Debug
"""
...
class JSONCommentImportProcessor:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.imported_files = []  # Список импортированных файлов для текущей обработки
    ...
    def process_imports(self, content: str, base_path: Path) -> Tuple[str, int, List[Path]]:
        ...
        Returns: (обработанный контент, количество импортов, список импортированных файлов)
        """
        import_count = 0
...
class JinjaHotReloaderV32(FileSystemEventHandler):
    ...
    def __init__(self, watch_dir: str = None, debug: bool = False,
        ...
        # НОВОЕ: Карта зависимостей child_file -> set(parent_files)
        ...
        logger.info(f"📊 Отслеживание зависимостей: ✅ Включено")
    ...
    def update_dependencies(self, parent_file: Path, imported_files: List[Path]):
        """
        Обновляет карту зависимостей

        Args:
            parent_file: Родительский файл, который импортирует другие
            imported_files: Список импортированных (дочерних) файлов
        """
        ...
        for child_file in imported_files:
            self.dependency_map[child_file].add(parent_file)

        if self.debug and imported_files:
            logger.debug(f"   📊 Зарегистрированы зависимости:")
            for child_file in imported_files:
                logger.debug(f"      {child_file.name} → {parent_file.name}")
    ...
    def get_parents_for_file(self, file_path: Path) -> Set[Path]:
        ...
        Args:
            file_path: Путь к дочернему файлу

        Returns:
            Множество родительских файлов
        """
    ...
    def process_jj_file(self, file_path: Path):
        ...
        try:
            ...
            cleaned_content, jinja_blocks, imported_files = self.preprocessor.clean_mixed_syntax(content, file_path)

            # 3. НОВОЕ: Обновляем карту зависимостей
            self.update_dependencies(file_path, imported_files)
            ...
            # 8. Рендеринг Jinja2 с smart контекстом
            json_str = json.dumps(json_obj, ensure_ascii=False)
    ...
    def on_modified(self, event):
        ...
        # НОВОЕ: Проверяем, является ли измененный файл дочерним (импортируемым)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Jinja Hot Reload v3.2.4 - Отслеживание зависимостей импортов'
    )
