...
def generate_output_paths(template_path):
    """
    Generate both MAP and FULL output paths from template path.
    [JJ_PC]_name.j2.java -> [MAP_PC]_name_web.json (with comments)
                         -> [FULL_PC]_name_web.json (clean JSON)
    [JJ_MOB]_name.j2.java -> [MAP_MOB]_name_web.json
                          -> [FULL_MOB]_name_web.json
    """
    ...
    if not match:
        ...
        map_path = os.path.join(template_dir, f"{base_name}_map.json")
        full_path = os.path.join(template_dir, f"{base_name}_output.json")
        return map_path, full_path

    platform = match.group(1)  # PC, MOB, etc.
    base_name = match.group(2)  # 1.0_main_screen_modular

    map_filename = f"[MAP_{platform}]_{base_name}_web.json"
    full_filename = f"[FULL_{platform}]_{base_name}_web.json"

    map_path = os.path.join(template_dir, map_filename)
    full_path = os.path.join(template_dir, full_filename)
...
def remove_json_comments(content):
    ...
    Removes lines that are pure comments (start with // after whitespace).
    Preserves strings and other content.
    """
    lines = content.split('\n')
    result = []

    for line in lines:
        # Check if line is a pure comment (only whitespace before //)
        stripped = line.lstrip()
        if stripped.startswith('//'):
            continue

        # Keep all other lines (including those with inline content)
        result.append(line)
...
def render_template(template_path, data_path, map_output_path, full_output_path):
    ...
    # 4. Write MAP Output (with module markers and comments)
    try:
        os.makedirs(os.path.dirname(map_output_path), exist_ok=True)

        with open(map_output_path, 'w', encoding='utf-8') as f:
            f.write(rendered)
        print(f"[{time.strftime('%H:%M:%S')}] 🗺️  MAP file written: {os.path.basename(map_output_path)}")
    except Exception as e:
        print(f"Error writing MAP output: {e}")
        return False

    # 5. Remove comments and write FULL Output (clean JSON for React)
    try:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] ✅ FULL file written: {os.path.basename(full_output_path)}")
        return True
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Jinja2 Hot Reload Script v3.8.0 with MAP/FULL dual output")
    parser.add_argument("--smart", action="store_true", help="Watch for changes in template and data files")
    args = parser.parse_args()

    # Generate both MAP and FULL output paths from template path
    map_path, full_path = generate_output_paths(TEMPLATE_PATH)

    print(f"Jinja2 Hot Reload v3.8.0 - Dual Output System")
    print(f"Input Template: {TEMPLATE_PATH}")
    print(f"Input Data:     {DATA_PATH}")
    print(f"MAP Output:     {map_path} (with module markers)")
    print(f"FULL Output:    {full_path} (clean JSON for React)")
    print("-" * 60)
    ...
    if args.smart:
        ...
        # Initial render
        if render_template(TEMPLATE_PATH, DATA_PATH, map_path, full_path):
            reload_browser()
